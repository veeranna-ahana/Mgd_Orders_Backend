            ...
            if (err) {
                ...
                if (data && data.length > 0) {
                    ...
                    if (schedule.Suspend === 1) {
                        return res.status(400).json({ message: "Clear Order Suspension of the order before trying to clear it for schedule" });
                            ...
                            if (err) {
                                ...
                                misQueryMod(suspendUpdateQuery, (err, result) => {
                                    if (err) {
                                        ...
                                        return res.status(200).json({ message: "Suspend status updated successfully" });
                                    }
                                });
                            }
                        });
                    ...
                    return res.status(404).json({ error: "No schedule found for the given ScheduleId" });
                }
            }
        });
    } catch (error) {
    ...
    try {
        misQueryMod(query, (err, data) => {
            if (err) {
                ...
                const isValid = data.every(detail => detail.QtyProduced === detail.QtyDelivered + detail.QtyRejected);
                ...
                if (isValid) {
                    try {
                        ...
                        updateOrderDetails(req.body.scheduleDetailsRow, (err, result) => {
                            if (err) {
                                ...
                                updateOrderSchedule(req.body.scheduleDetailsRow, (err, result) => {
                                    if (err) {
                                        console.log("err", err);
                                        next(err); // Pass the error to the error handling middleware
                                    } else {
                                        // Execute the final update query
                                        updateNCTaskList(req.body.scheduleDetailsRow, (err, result) => {
                                            if (err) {
                                                console.log("err", err);
                                                next(err); // Pass the error to the error handling middleware
                                            } else {
                    ...
                    res.send("Either all quantity produced must be dispatched or balance quantity must be recorded as 'Rejected'");
                }
            }
        });
    } catch (error) {
            ...
            if (err) {
                ...
                if (data && data.length > 0) {
                    ...
                    if (resultQuery.QtyProgrammed > 0) {
                                            ...
                                            if (err) {
                                                ...
                                                misQueryMod(deleteQuery, (err, result4) => {
                                                    if (err) {
                                                        ...
                                                        return res.status(200).json({ message: "Schedules cancelled successfully" });
                                                    }
                                                });
                                            }
                                        });
                        ...
                        return res.status(400).json({ message: "Cannot Cancel Schedules Once Programmed" });
                    }
                } else {
                    return res.status(404).json({ error: "No data found for the given SchDetailsID" });
                }
            }
        });
    } catch (error) {
                ...
                if (salesOverdueCount > 0) {
                    ...
                    misQueryMod(queryPaymentCaution, (err, paymentCautionData) => {
                        if (err) {
                            ...
                            const paymentCautionCount = paymentCautionData[0].PaymentCautionCount;
