          ...
          if ((req.body.scheduleOption===' Full Order'  && count === req.body.selectedItems.length) || req.body.scheduleOption==='Partial Order') {
            ...
            misQuery(insertquery1, (result, err1) => {
              if (err1) {
                ...
                return res.status(500).json({ message: "Error occurred while inserting into orderschedule" });
              } else {
            ...
            return res.status(200).json({ message: "You do not have any schedulable serial" });
          }
        }
      });
    } catch (error) {
...
ProductionSchCreationRouter.post(`/schedulelistbasedonScheduleType`, async (req, res, next) => {
