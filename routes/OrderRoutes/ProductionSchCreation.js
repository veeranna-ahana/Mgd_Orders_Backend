...
ProductionSchCreationRouter.post(`/createProductionSchedule`, async (req, res, next) => {
  ...
  let insertquery1 = `INSERT INTO magodmis.orderschedule(Order_No, PO, Cust_Code, ScheduleDate, Delivery_date, 
                ...
                VALUES ('${req.body.OrderData.Order_No}','${req.body.OrderData.Purchase_Order}','${req.body.OrderData.Cust_Code}',now(),adddate(curdate(), INTERVAL 2 DAY), '${req.body.OrderData.SalesContact}', '${req.body.OrderData.Dealing_Engineer}','${req.body.OrderData.Type === 'Service' ? req.body.OrderData.Type : req.body.scheduleType}','${req.body.OrderData.Type}','0','Created',now())`;
          ...
          if (err) {
            ...
            if (resultQty.length > 0) {
              ...
              let inserquery2 = `INSERT INTO magodmis.orderscheduledetails(ScheduleId, OrderDetailID, Order_No,
                ...
                '${req.body.scheduleType === 'Job Work' ? 'Customer' : 'Magod'}','${selectedItem.PackingLevel}', '${selectedItem.InspLevel}','${QtyToSchedule}','${selectedItem.tolerance}','${selectedItem.JWCost}','${selectedItem.MtrlCost}','${selectedItem.HasBOM}')`;
